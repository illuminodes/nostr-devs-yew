...
pub fn app_layout(props: &html::ChildrenProps) -> Html {
    html! {
        <div class="flex flex-col min-h-screen font-source overflow-hidden h-screen">
            <AppHeader />
            <main class="flex-1 container px-4 sm:px-0 sm:mx-auto py-4 lg:py-8 overflow-hidden">{props.children.clone()}</main>
            <AppFooter />
        </div>
    }
}
        ...
        <header class="p-4 sm:p-6 md:p-8">
            <div class="container mx-auto flex justify-between items-center">
                <AppLink route={AppRoute::Home}>
                    <div class="flex flex-row gap-2 sm:gap-4 md:gap-8 items-center">
                        <img src="/public/assets/illumGray.png" alt="NostrDevs" class="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-16 lg:h-16" />
                        <h3 class={header.to_class()}>
                            {"NostrDevs SV"}
                        </h3>
                    </div>
                </AppLink>
                <div class="flex items-center space-x-4">
                    <a href={LATEST_MEETUP_URL} target="_blank" rel="noopener noreferrer"
                        class="hover:underline">
                        <h4 class={text.to_class()}>{"RSVP"}</h4>
                    </a>
                </div>
            </div>
        </header>
...
pub fn app_footer() -> Html {
    html! {
        <footer class="p-4 sm:p-6 md:p-8">
            <div class="container mx-auto flex justify-center space-x-8 md:space-x-12 lg:space-x-16">
                <a href={X_URL} target="_blank" rel="noopener noreferrer">
                    <Twitter class="w-8 h-8 fill-white text-white" />
                </a>
                <a href={GITHUB_URL} target="_blank" rel="noopener noreferrer">
                    <Github class="w-8 h-8 fill-white text-white" />
                </a>
                <a href={PRIMAL_URL} target="_blank" rel="noopener noreferrer">
                    <NostrIcon />
                </a>
           </div>
        </footer>
...
#[function_component(NostrIcon)]
pub fn nostr_icon() -> Html {
    let class = "w-8 h-8 text-white fill-white";
    html! {
        <svg {class} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 875 875">
            <path
                class="cls-1"
